...
启动后会先出现**模式菜单**：可选「局域网双人（自动发现配对）」或「单机游戏」，无需命令行参数。
...
两台电脑都运行 `python main.py`（或双击 `run_game.bat`），在菜单中选 **局域网双人（自动发现配对）**，按提示选择会话或等待配对即可。

### 命令行（可选，脚本/调试用）

- 直接进配对界面：`python main.py --lan auto`
- 直接单机：`python main.py --lan off`
- 主机：`python main.py --lan host --bind-ip 0.0.0.0 --port 28990`
- 客户端：`python main.py --lan client --host-ip 192.168.x.x --port 28990`

联机操作：两边都用 **WASD/方向键 + 空格**；主机为 P1，客户端为 P2。
